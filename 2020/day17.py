...
for ix in range(0,nr):
	for iy in range(0,nc):
		G[(ix,iy,0,0)] = True if inp[ix][iy]=='#' else False
		...
		for iy in range(y-1,y+2):
			for iz in range(z-1,z+2):
				for iw in range(w-1,w+2):
					if G[(ix,iy,iz,iw)] and (ix,iy,iz,iw)!=(x,y,z,w):
						n += 1
	return n
...
def Grange(G):
	...
	for g in tmp:
		...
		y1,y2 = min(y1,g[1]), max(y2,g[1])
		z1,z2 = min(z1,g[2]), max(z2,g[2])
		w1,w2 = min(w1,g[3]), max(w2,g[3])
	return ((x1,x2),(y1,y2),(z1,z2),(w1,w2))

nc = 6
for i in range(nc):
	G2 = G.copy() # shallow copy is ok since values are scalars
	xx,yy,zz,ww = Grange(G)
	for ix in range(xx[0]-1,xx[1]+2):
		for iy in range(yy[0]-1,yy[1]+2):
			for iz in range(zz[0]-1,zz[1]+2):
				for iw in range(ww[0]-1,ww[1]+2):
					if not (G[(ix,iy,iz,iw)]==True and numnear(ix,iy,iz,iw) in (2,3)):
						G2[(ix,iy,iz,iw)] = False
					if (G[(ix,iy,iz,iw)]==False) and (numnear(ix,iy,iz,iw)==3):
						G2[(ix,iy,iz,iw)] = True
	G = G2.copy()

print("Part 2: {}".format(sum(G.values())))
